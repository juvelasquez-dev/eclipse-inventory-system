-- Require an ACTIVE ADMIN caller for all administrative user-management
-- mutations. Target-user status checks remain in the individual functions.
...
BEGIN
  ...
  IF v_role IS DISTINCT FROM 'ADMIN'
    ...
    RAISE EXCEPTION 'Access denied: active admin role required';
  END IF;
END;
$function$;

REVOKE ALL ON FUNCTION public.require_active_admin() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.require_active_admin() FROM anon;
GRANT EXECUTE ON FUNCTION public.require_active_admin() TO authenticated;
